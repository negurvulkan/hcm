...
if ($isAdmin) {
    ...
    $currentStart = db_first("SELECT si.position, si.start_number_display, p.display_name AS rider, h.name AS horse, c.label AS class_label FROM startlist_items si JOIN entries e ON e.id = si.entry_id JOIN parties p ON p.id = e.party_id JOIN horses h ON h.id = e.horse_id JOIN classes c ON c.id = si.class_id WHERE si.state = 'running' ORDER BY si.updated_at DESC LIMIT 1");
    $nextStarters = db_all("SELECT si.position, si.start_number_display, p.display_name AS rider FROM startlist_items si JOIN entries e ON e.id = si.entry_id JOIN parties p ON p.id = e.party_id WHERE si.state = 'scheduled' ORDER BY si.planned_start ASC, si.position ASC LIMIT 5");
} else {
    if (!$activeEvent) {
        $openEntries = 0;
        $paidEntries = 0;
        ...
        $currentStart = db_first("SELECT si.position, si.start_number_display, p.display_name AS rider, h.name AS horse, c.label AS class_label FROM startlist_items si JOIN entries e ON e.id = si.entry_id JOIN parties p ON p.id = e.party_id JOIN horses h ON h.id = e.horse_id JOIN classes c ON c.id = si.class_id WHERE si.state = 'running' AND e.event_id = :event_id ORDER BY si.updated_at DESC LIMIT 1", $countParams);
        $nextStarters = db_all("SELECT si.position, si.start_number_display, p.display_name AS rider FROM startlist_items si JOIN entries e ON e.id = si.entry_id JOIN parties p ON p.id = e.party_id WHERE si.state = 'scheduled' AND e.event_id = :event_id ORDER BY si.planned_start ASC, si.position ASC LIMIT 5", $countParams);
    }
}
